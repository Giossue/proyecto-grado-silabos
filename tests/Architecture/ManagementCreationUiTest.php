...
it('prepara solo materias pendientes en una hoja lateral amplia', function (): void {
    ...
    expect($sheet)
        ...
        ->toContain('row.selected = $event === true')
        ->toContain('selected: false')
        ->toContain('preparedSubjectIds')
        ->toContain('planningPeriods')
        ->toContain('initialPeriodId')
...
it('construye la plantilla desde bloques que contienen campos tipados', function (): void {
    ...
    expect($editor)
        ...
        ->toContain('id="template-editor-header-selection"')
        ->toContain('selection-target="#template-editor-header-selection"')
        ->not->toContain('{{ template.name }}')
        ->not->toContain('Modo edición')
    ...
    expect($builder)
        ...
        ->toContain('template.titleBlock.text')
        ->toContain('data-template-selectable="title"')
        ->toContain('Editar título')
        ->toContain('<TemplateTitleActions')
        ->not->toContain('>PROGRAMA DE ASIGNATURA (SÍLABO)<')
        ...
        ->toContain(':to="selectionTarget ?? \'body\'"')
        ->toContain(':disabled="!selectionTarget"')
        ->toContain('id="template-editor-ribbon-actions"')
        ->toContain('class="flex min-w-0 flex-1 items-center justify-end gap-2 overflow-x-auto"')
        ->not->toContain('<div class="min-w-0 flex-1" aria-hidden="true" />')
        ...
        ->toContain('<TemplateToolbarButton')
        ->toContain('TemplateController.reorderSections.url')
        ->toContain('TemplateController.reorderBlocks.url')
        ->toContain('label="Mover campo arriba"')
        ->toContain('label="Mover campo abajo"')
        ->toContain('label="Mover bloque arriba"')
        ->toContain('label="Mover bloque abajo"')
        ->toContain('@pointerdown.capture="clearSelectionFromBackground"')
        ->toContain(':role="readonly || activeTable ? undefined : \'button\'"')
        ->toContain('data-template-selectable="section"')
        ->toContain('data-template-selectable="field"')
        ->toContain('class="relative mb-6 rounded-none outline-offset-4 transition-shadow"')
        ->toContain('class="relative mb-4 scroll-mt-40 rounded-none outline-offset-4 transition-shadow"')
    ...
    expect($tableDesigner)
        ...
        ->toContain('<Teleport')
        ->not->toContain('Editando:')
        ->not->toContain('Seleccione celdas y use la cinta superior para dar formato.')
    ...
    expect($tableEditor)
        ...
        ->toContain('Color de toda la celda')
        ->toContain('Color de fuente')
        ->toContain('chain?.setColor(value).run()')
        ->toContain('chain?.unsetColor().run()')
        ->toContain('Alineación de celda')
        ->toContain('Tamaño de fuente de celda')
        ->toContain('fontSize')
        ...
        ->toContain('Estructura de filas')
        ->not->toContain('v-if="repeatField"')
        ->toContain('repeatLabel: { default: null }')
        ->toContain('Organizar por unidades')
        ->toContain('Fila que completa el docente')
        ->not->toContain('rowRoleLabel')
        ...
        ->toContain('View: ResponsiveTableView')
        ->toContain('class="template-table-canvas min-w-0 overflow-x-clip"')
        ->toContain('cursor: text;')
        ->not->toContain('min-width: 580px')
        ->toContain('<DropdownMenu');
    expect($inlineNode)
        ->toBeString()
    ...
    expect($appearance)
        ...
        ->toContain('Actualizar logo')
        ->toContain('Logo de la universidad')
        ->toContain('institutionLogoUrl')
        ->toContain('class="sr-only"')
        ->toContain('@change="uploadLogo"')
        ->toContain('<Button v-else as-child variant="outline">')
        ->toContain('<label for="template-institution-logo">')
        ->not->toContain('@click="chooseLogo"')
        ->not->toContain('PNG transparente. Recomendado')
